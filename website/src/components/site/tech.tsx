import { useTranslations } from "next-intl";
import { Reveal } from "./reveal";
import { Brain, Mic, Radio, MapPin, Bike, ShieldCheck } from "lucide-react";

const ICONS = [Brain, Mic, Radio, MapPin, Bike, ShieldCheck];

export function Tech() {
  const t = useTranslations("tech");
  const items = t.raw("items") as { title: string; desc: string }[];

  return (
    <section className="scroll-mt-24 py-20 md:py-28">
      <div className="container-x">
        <Reveal className="mx-auto max-w-2xl text-center">
          <span className="eyebrow mb-4 justify-center">{t("eyebrow")}</span>
          <h2 className="text-[clamp(1.8rem,4vw,2.75rem)] font-bold leading-tight">
            {t("title")}
          </h2>
          <p className="mx-auto mt-4 max-w-xl text-[17px] leading-relaxed text-ink-soft">
            {t("subtitle")}
          </p>
        </Reveal>

        <div className="mt-12 grid gap-x-8 gap-y-9 sm:grid-cols-2 lg:grid-cols-3">
          {items.map((item, i) => {
            const Icon = ICONS[i] ?? Brain;
            return (
              <Reveal key={i} delay={0.04 * i}>
                <div className="flex gap-4">
                  <span className="flex h-11 w-11 shrink-0 items-center justify-center rounded-[14px] bg-[color-mix(in_srgb,var(--accent)_12%,transparent)] text-accent">
                    <Icon className="h-5 w-5" />
                  </span>
                  <div>
                    <h3 className="text-[16px] font-semibold">{item.title}</h3>
                    <p className="mt-1.5 text-[15px] leading-relaxed text-ink-soft">
                      {item.desc}
                    </p>
                  </div>
                </div>
              </Reveal>
            );
          })}
        </div>
      </div>
    </section>
  );
}
